...
interface TravelGuidesProps {
  initialGuides?: TourismItem[];
}

export function TravelGuides({ initialGuides = [] }: TravelGuidesProps) {
  const [items, setItems] = useState<TourismItem[]>(initialGuides);
  const [loading, setLoading] = useState(initialGuides.length === 0);
      ...
      <div className="container mx-auto px-4">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {items.map((guide) => (
              <article
                key={guide.id}
                className="bg-card rounded-2xl overflow-hidden shadow-lg group hover:shadow-xl transition-shadow border border-border/50 flex flex-col"
              >
        ...
        {!loading && items.length === 0 && (
          <p className="text-center text-muted-foreground">
            Hiện chưa có thông tin du lịch nào.
          </p>
        )}
